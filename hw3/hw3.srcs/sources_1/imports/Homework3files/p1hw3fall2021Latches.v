...
// Output logic
always @ (CurrentState)
	if (CurrentState == State1010) Z <= 1; else Z <= 0;
...
always @ (CurrentState, X)
	case (CurrentState)
		...
		State01: if (X==1) NextState <= State1; else NextState <= State01;
		State10: if(X==0) NextState <= State1010; else NextState <= State011;
		State1010: if(X==0) NextState <= State01;  else NextState <= State1010;
